use crate::{
    impl_add_sub_from_field_ref, impl_mul_div_from_field_ref, FftField, Field, FieldError,
    FieldParameters, LegendreSymbol, One, PoseidonDefaultField, PoseidonDefaultParameters,
...
impl<P: Fp256Parameters> Field for Fp256<P> {
    ...
    fn sum_of_products<'a>(
        ...
        let (u0, u1, u2, u3) = (0..4).fold((0, 0, 0, 0), |(u0, u1, u2, u3), j| {
            ...
            let (t0, t1, t2, t3, mut t4) = a.clone().zip(b.clone()).fold(
                (u0, u1, u2, u3, 0),
                |(t0, t1, t2, t3, mut t4), (a, b)| {
...
impl<P: Fp256Parameters> FromStr for Fp256<P> {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let ten = Self::from_repr(<Self as PrimeField>::BigInteger::from(10))
