    ...
    ($field: ident, $params: ident, $byte_size: expr) => {
        impl<P: $params> CanonicalSerializeWithFlags for $field<P> {
            ...
            fn serialize_with_flags<
                W: snarkvm_utilities::io::Write,
                F: snarkvm_utilities::Flags,
            >(
                &self,
                mut writer: W,
                flags: F,
                ...
                let output_byte_size =
                    number_of_bits_and_bytes(P::MODULUS_BITS as usize + F::BIT_SIZE).1;
            ...
            fn serialized_size_with_flags<F: snarkvm_utilities::Flags>(&self) -> usize {
                snarkvm_utilities::serialize::number_of_bits_and_bytes(
        ...
        impl<P: $params> $field<P> {
            const SERIALIZED_SIZE: usize =
                snarkvm_utilities::serialize::number_of_bits_to_number_of_bytes(
                                ...
                                let bytes = (0..len)
                                    .map(|_| {
                                        seq.next_element()?.ok_or_else(|| {
                                            serde::de::Error::custom("could not read bytes")
                                        })
                                    })
...
macro_rules! impl_field_from_random_bytes_with_flags {
    ($u64_limbs: expr) => {
        #[inline]
        fn from_random_bytes_with_flags<F: snarkvm_utilities::Flags>(
            bytes: &[u8],
        ) -> Option<(Self, F)> {
            (F::BIT_SIZE <= 8)
                .then(|| {
