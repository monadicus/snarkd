...
use snarkvm_fields::{
    fp6_3over2::Fp6Parameters, Field, Fp12, Fp12Parameters, Fp2, Fp2Parameters, One, PrimeField,
...
pub trait Bls12Parameters:
    'static + Copy + Clone + Debug + PartialEq + Eq + Send + Sync + Sized
{
    const X: &'static [u64];
    ...
    fn g1_is_in_correct_subgroup(
        p: &short_weierstrass_jacobian::Affine<Self::G1Parameters>,
    ) -> bool {
        p.mul_bits(BitIteratorBE::new(
            <Self::G1Parameters as ModelParameters>::ScalarField::characteristic(),
        ))
        .is_zero()
    }

    fn g2_is_in_correct_subgroup(
        p: &short_weierstrass_jacobian::Affine<Self::G1Parameters>,
    ) -> bool {
        p.mul_bits(BitIteratorBE::new(
            <Self::G1Parameters as ModelParameters>::ScalarField::characteristic(),
