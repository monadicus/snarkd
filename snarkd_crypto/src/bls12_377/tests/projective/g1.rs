...
pub type G1ProjectiveTuple = ProjectiveTuple<Fp>;
...
impl Namespace for G1ProjectiveNs {
    fn run_test(&self, test: Test) -> TestResult {
        match test.method.as_str() {
            "add" => {
                let (a, b, c): (G1ProjectiveTuple, G1ProjectiveTuple, G1ProjectiveTuple) =
                    Self::get(test.input);
                add::<G1Projective>(a.into(), b.into(), c.into())
            }
            "mul" => {
                let (a, b, s): (G1ProjectiveTuple, G1ProjectiveTuple, _) = Self::get(test.input);
                mul::<G1Projective>(a.into(), b.into(), s)
            }
            "double" => {
                let (a, b): (G1ProjectiveTuple, G1ProjectiveTuple) = Self::get(test.input);
                double::<G1Projective>(a.into(), b.into())
            }
            "neg" => {
                let (a, s): (G1ProjectiveTuple, _) = Self::get(test.input);
                neg::<G1Projective>(a.into(), s)
            }
            "transform" => {
                let g: G1ProjectiveTuple = Self::get(test.input);
                transform::<G1Projective>(g.into())
            }
            "batch_normalization" => {
                let batch: Vec<G1ProjectiveTuple> = Self::get(test.input);
                let batch = batch.into_iter().map(|v| v.into()).collect();
                batch_normalization::<G1Projective>(batch)
            }
            "projective_glv" => {
                let (point, scalar): (G1ProjectiveTuple, _) = Self::get(test.input);
