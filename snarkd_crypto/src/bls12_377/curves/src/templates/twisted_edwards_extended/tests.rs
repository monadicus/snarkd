...
use snarkvm_utilities::{
    ...
    to_bytes_le, Compress, TestRng, ToBytes, Validate,
};
    ...
    P: TwistedEdwardsParameters,
{
    ...
    let a = P::BaseField::one().double()
        * (P::EDWARDS_A + P::EDWARDS_D)
        * (P::EDWARDS_A - P::EDWARDS_D).inverse().unwrap();
    // B = 4 / (a - d)
    let b =
        P::BaseField::one().double().double() * (P::EDWARDS_A - P::EDWARDS_D).inverse().unwrap();
    ...
    P::BaseField: PrimeField,
{
    ...
    for _ in 0..ITERATIONS {
        let biginteger =
            <<Affine<P> as AffineCurve>::BaseField as PrimeField>::BigInteger::rand(rng);
